...
export default class NobreakController {
  ...
  static async store(req: Request, res: Response) {
    ...
    if (funcionario.nobreak) {
      return res.status(400).json({ error: 'O funcionário já possui um Nobreak associado' });
    }
  ...
  static async update(req: Request, res: Response) {
    ...
    if (!funcionario.nobreak) {
      return res.status(400).json({ error: 'O funcionário não possui um Nobreak associado' });
    }
